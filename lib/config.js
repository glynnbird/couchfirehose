...
const config = {
  ...
  SELECTOR: process.env.SELECTOR || null
}
...
const args = require('yargs')
  ...
  .option('selector', { type: 'string', describe: 'Selector applied to the source\'s changes feed for filtering', default: config.SELECTOR })
  .help('help')
  .argv
...
if (config.SELECTOR) {
  try {
    config.SELECTOR = JSON.parse(config.SELECTOR)
    assert(typeof config.SELECTOR === 'object')
  } catch (e) {
    throw new Error('SELECTOR must parse to a JavaScript Object')
  }
}
